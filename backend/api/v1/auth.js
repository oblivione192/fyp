...
userRouter.post('/checkTokenExpiry',async function(req,res){
   ...
   jwt.verify(req.body.token,process.env.COOKIE_SECRET,function(err,decoded){
     if(err){ 
...
userRouter.post('/login',async function(req,res){  
  ...
  if(statusCode == 2){ 
    ...
    var token = null; 
    if(rememberMe){token = jwt.sign({user_id:user_id},process.env.COOKIE_SECRET,{expiresIn: 86400*365.25})}
    else{token = jwt.sign({user_id:user_id},process.env.COOKIE_SECRET)}
